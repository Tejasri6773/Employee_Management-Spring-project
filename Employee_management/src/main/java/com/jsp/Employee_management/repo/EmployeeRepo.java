...
//	Optional<EmployeeDetails> findByEmail(String email);
	@Query("select a from EmployeeDetails a where email=?1")
	...
	@Query("select a from EmployeeDetails a where email=?1 and pwd=?2")
	EmployeeDetails findByEmailPwd(String email,String pwd);
