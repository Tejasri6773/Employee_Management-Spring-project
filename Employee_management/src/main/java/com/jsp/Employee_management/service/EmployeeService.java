...
//sending email through spring boot
	public void sendHtmlEmail(EmployeeDetails emp) throws MessagingException {
		...
		message.setFrom(new InternetAddress("tejasri30zz@gmail.com"));
...
//login operation
	...
	public ResponseEntity<ResponseStructure<EmployeeDetails>> fetchemailpwd(String email,String pwd){
		EmployeeDetails e=dao.empFetch(email);
		if(e!=null) {
			EmployeeDetails p=dao.empFetchByPwd(pwd);
			if(p!=null) {
				ResponseStructure<EmployeeDetails> rs=new ResponseStructure<EmployeeDetails>();
				rs.setData(mapper.map(e,EmployeeDetails.class));
				...
				throw new PwdNotFound("password incorrect");
		}
		else
